...
public class CalendarPop extends JPopupMenu{
	...
	public CalendarPop() {
		...
		buildCalendarPanel(initCalendar);
	 ...
	 * 构建日历控件
	 */
	private void buildCalendarPanel(Calendar calendar2) {
		...
		do {
			...
			//如果为当前的日期，则显示一个边框
			if (calendar2.get(Calendar.YEAR)==calender.get(Calendar.YEAR)&&
					calendar2.get(Calendar.MONTH) == calender.get(Calendar.MONTH)&&
							calendar2.get(Calendar.DAY_OF_MONTH)==calender.get(Calendar.DAY_OF_MONTH)) {
		...
		int weekDay2 = calender.get(Calendar.DAY_OF_WEEK);
		for (int j = weekDay2; j < 7; j++) {
			calendarPanel.add(new DayPanel(""));
		}
	}
