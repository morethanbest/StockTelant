...
public class KLine {
	...
	private static KType p;

	public void setdata(double[][] data, double ymin, double ymax, int length, LocalDate start, LocalDate end,
			KType p) {
		this.data = data;
		this.ymin = ymin;
		this.ymax = ymax;
	...
	protected CandleStickChart createChart() {
		...
		final NumberAxis xAxis = new NumberAxis(0, length + 1, 1);
		...
		// setup chart
		if (p == KType.DAY)
			xAxis.setLabel(start.toString()
					+ "                                "
					+ LocalDate.ofEpochDay(end.toEpochDay() - 2 * length / 3).toString()
					+ "                                "
					+ LocalDate.ofEpochDay(end.toEpochDay() - length / 3).toString()
					+ "                                " + end.toString());
		else if (p == KType.WEEK)
			xAxis.setLabel(start.toString()
					+ "                                "
					+ LocalDate.ofEpochDay(end.toEpochDay() - 14 * length / 3).toString()
					+ "                                "
					+ LocalDate.ofEpochDay(end.toEpochDay() - 7 * length / 3).toString()
					+ "                                " + end.toString());
		else if (p == KType.MONTH)
			xAxis.setLabel(start.toString().substring(0, 7)
					+ "                                   "
					+ start.plusMonths(length / 3).toString().substring(0, 7)
					+ "                                   "
					+ start.plusMonths(2 * length / 3).toString().substring(0, 7)
		...
		for (int i = 0; i < data.length; i++) {
			...
			series.getData().add(
					...
					new XYChart.Data<Number, Number>(day[0], day[1],
	 ...
	 * price movements of a security, derivative,
	 *
	 ...
	 * and low values are stored in the Data's
	 *
				...
				while (iter.hasNext()) {
					...
					if (itemNode instanceof Candle && extra != null) {
						...
						if (getXAxis() instanceof NumberAxis) {
							...
							candleWidth = xa.getDisplayPosition(xa.getTickUnit()) * 0.5; // use
																							// 90%
																							// width
																							// between
																							// ticks
